...
class TopKHeap:
    ...
    def insert(self, elt):
        ...
        if elt < self.A[len(self.A)-1]:
            ...
            self.H.insert(last)
            self.A[len(self.A)-1] = elt
            next1 = len(self.A)-2
            curr = len(self.A)-1
            while curr > 0:
                if self.A[next1] > self.A[curr]:
                    self.A[curr], self.A[next1] = self.A[next1],self.A[curr]
                    curr = curr - 1
                    next1 = next1 - 1
                else:
                    return
    ...
    def delete_top_k(self, j):
        ...
        assert j < self.k
        to_delete = self.A[j]
        top_heap = self.H.min_element()
        if to_delete < top_heap:
            self.A.append(top_heap)
            self.H.insert(to_delete)
            self.H.delete_min()
        else:
            print('here1')
            self.A.pop(j)
            self.insert(top_heap)
            self.H.delete_min()
        self.H.delete_min()
        self.A.pop(j)
